...
**链路① Docker 一键启动**（评委 / 演示）——只需 Docker Desktop：

```bash
docker compose up --build        # 然后浏览器打开 http://localhost/
```

完整指引、加速参数、mock RPA 叠加、无数据包预览、撞坑速查表：
**[docs/快速上手_Docker链路.md](docs/快速上手_Docker链路.md)**

**链路② 本地源码开发**（队友 / 二次开发）——需 Python 3.11+ 与 Node 20+：

```bash
pip install -r requirements.txt
python -m uvicorn app.api:app --host 127.0.0.1 --port 8000   # 后端
cd web && npm ci && npm run dev                              # 前端（另开终端，5173）
```

知识库索引**首次启动自动构建**（约 30 秒），手动重建 `python -m app.kb`。
完整指引、逐字收录的报错速查表（含队友实测三连坑）：
**[docs/快速上手_本地开发链路.md](docs/快速上手_本地开发链路.md)**

两条链路共用：

- 考题数据包放仓库**同级目录** `../创灵境_考题模拟数据`（保密数据不入库），或设 `COST_INSIGHT_DATA_DIR`
- LLM 接入（可选）：仓库根放 `.env` 写 `LLM_BASE_URL / LLM_API_KEY / LLM_MODEL`。
  **不配也能跑**——意图路由降级规则兜底、报告降级模板生成，界面如实标注降级层，不放假数据
- 报告模板属保密资源不入库：Docker 启动自动从数据包复制；本地开发手动复制到
  `assets/report_template.docx`（缺模板时报告接口给中文报错，其余功能不受影响）
